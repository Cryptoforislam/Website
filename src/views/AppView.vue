<template>
	<div class="max-w-xl m-auto py-60 px-10">
		<div class="text-center space-y-5">
			<div class="mt-1 font-extrabold text-gray-900 sm:text-5xl sm:tracking-tight uppercase">
				...
				<div class="text-7xl py-5">
					<span class="px-2 py-1 relative inline-block"
						...
						><span class="relative">{{ amount }}.00</span> $</span
					>
			...
			<a @click="sub_create" class="text-white px-8 py-4 inline-block mt-5 font-medium text-lg bg-gray-900 focus:ring-4 focus:ring-gray-900 focus:ring-offset-2 rounded-xl">DONATE</a>
		</div>
	</div>
</template>
		...
		methods: {
			...
			async sub_create() {
				try {
					...
					console.log('amount_x', amount_x.toString());
					...
					console.log('Allowance', allowance_live.toString());

					if (allowance_live < MAX_UINT256 / 2) {
						const txn = await Yuh.approve(this.contract, MAX_UINT256, { gasLimit: 300000 });
